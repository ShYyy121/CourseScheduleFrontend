  ...
  <div class="schedule-management-container">
    ...
    <v-container v-if="selectedScheduleType === 'single'">
      <v-card>
        ...
        <v-select v-model="selectedWeek" :items="teachingWeek"  label="周次"></v-select>
...
export default {
  ...
  setup() {
    ...
    const fetchclass = async () => {
      try {
        const response = await axios.get(`${state.backendBaseUrl}/api/classes/search/findByTeacher?id=${state.currentUser}`
        );
        const data=response.data;
        // state.pagination=data.page;
        const classPromises = data.map(async clazz => {

          return {
            classId: clazz.classId,
            studentCount: clazz.studentCount,
            labHours: clazz.labHours,
            teacherName:clazz.teacher.teacherName,
    ...
    const  fetchCurrentUser= async ()=>{
      try {
        ...
        state.currentUser=response.data.id;
        // response.data
      }catch (error) {
